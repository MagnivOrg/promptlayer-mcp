...
| `create-smart-table` | Create a Smart Table. Default creates Sheet 1 (`create_default_sheet=true`). Set `create_default_sheet=false` to skip it. Response includes `default_sheet: { id, title }` when a default sheet is created. |
...
| `add-smart-table-rows` | Add rows. Use `count` for N blank shells (max 100). Optional `values` are `[{column_uuid: rawText}, ...]` for TEXT columns only. |
